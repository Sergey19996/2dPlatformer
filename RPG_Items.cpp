...
cEquip::cEquip(std::string name, olc::Decal* sprite, std::string desc, uint8_t dmg, uint8_t aglty, uint8_t inteleg, uint8_t versality, uint8_t strength, uint32_t sprtindex, uint8_t equipindex, uint8_t price ) : cItem(name, sprite, desc)
{
	...
	Attributes |= (aglty & 0xF) << 0;       // Записываем agility в первые 4 бита (0-3)
	Attributes |= (inteleg & 0xF) << 4;     // Записываем intelligence в следующие 4 бита (4-7)
	Attributes |= (versality & 0xF) << 8;   // Записываем versatility в биты 8-11
	Attributes |= (strength & 0xF) << 12;   // Записываем strength в биты 12-15
	Attributes |= (dmg & 0xF) << 16;        // Записываем DMg в биты 16-20
